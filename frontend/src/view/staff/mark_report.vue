    ...
    <div class="table-container">
      <table>
        ...
        <tr v-for="(student, index) in students" :key="index">
          ...
          <td class="center">{{ student.avgMark.toFixed(2) }}</td>
          <td class="center">
            <VsxIcon
                iconName="Eye"
                :size="30"
...
export default {
  ...
  methods: {
    ...
    async fetchStudentsByClassId(classId) {
      ...
      try {
        ...
        if (result && result.content) {
          ...
          await this.calculateStudentsMarks(classId);
        } else {
          this.students = [];
          console.error('Unexpected response format', response.data);
        }
    ...
    async calculateStudentsMarks(classId) {
      try {
        const token = sessionStorage.getItem('jwtToken');
        const response = await axios.post(
            `http://localhost:8088/fja-fap/staff/calculate-students-marks?classId=${classId}`,
            {}, // Body trống nếu API không yêu cầu dữ liệu
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        if (response.data && response.data.success) {
          this.notification = {
            message: "Student marks calculated successfully!",
            type: "success",
          };
        } else {
          console.error('Error calculating student marks', response.data);
          this.notification = {
            message: "Failed to calculate student marks.",
            type: "error",
          };
        }
      } catch (error) {
        console.error('Error calling calculate-students-marks API:', error);
        this.notification = {
          message: "Error occurred while calculating student marks.",
          type: "error",
        };
      }
    },
    ...
    viewStudentAttendanceReport(studentId) {
      this.$router.push({ name: 'StaffStudentMarkReport', params: { id: studentId } });
    }
  },
  mounted() {
    this.fetchBatches();
